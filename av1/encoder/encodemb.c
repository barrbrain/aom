...
static int cfl_compute_alpha_ind(MACROBLOCK *const x, const CFL_CTX *const cfl,
                                 BLOCK_SIZE bsize, uint8_t y_pix[MAX_SB_SQUARE],
                                 int *mag_out) {
  ...
  sse[CFL_PRED_U][0] =
      cfl_alpha_dist(y_pix, MAX_SB_SIZE, y_avg, src_u, src_stride_u,
                     block_width, block_height, dc_pred_u, 0, NULL);
  sse[CFL_PRED_V][0] =
      cfl_alpha_dist(y_pix, MAX_SB_SIZE, y_avg, src_v, src_stride_v,
  ...
  for (int m = 1; m < CFL_MAGS_SIZE; m += 2) {
    ...
    sse[CFL_PRED_U][m] = cfl_alpha_dist(
        y_pix, MAX_SB_SIZE, y_avg, src_u, src_stride_u, block_width,
        block_height, dc_pred_u, cfl_alpha_mags[m] * q15,
        &sse[CFL_PRED_U][m + 1]);
    sse[CFL_PRED_V][m] = cfl_alpha_dist(
        y_pix, MAX_SB_SIZE, y_avg, src_v, src_stride_v, block_width,
...
void av1_predict_intra_block_encoder_facade(MACROBLOCK *x,
  ...
  if (plane != AOM_PLANE_Y && mbmi->uv_mode == UV_CFL_PRED) {
    if (blk_col == 0 && blk_row == 0 && plane == AOM_PLANE_U) {
      ...
      mbmi->cfl_uvec_idx = cfl_compute_alpha_ind(x, cfl, plane_bsize, tmp_pix,
                                                 &mbmi->cfl_mag_idx);
    }
  }
#if CONFIG_DEBUG
