...
void fdct4(const tran_low_t *x, tran_low_t *y) {
  int t0;
  int t1;
  int t2;
  int t3;
...
void fadst8(const tran_low_t *x, tran_low_t *y) {
  int t0;
  int t1;
  int t2;
  int t3;
...
void av1_fht4x4_c(const int16_t *input, tran_low_t *output, int stride,
...
#else
  {
    ...
    for (i = 0; i < 4; ++i) {
#if CONFIG_DAALA_TX
      for (j = 0; j < 4; ++j) temp_in[j] = input[j * stride + i] << 3;
#else
...
void av1_fht8x8_c(const int16_t *input, tran_low_t *output, int stride,
...
#else
  {
    ...
    for (i = 0; i < 8; ++i) {
#if CONFIG_DAALA_TX
      for (j = 0; j < 8; ++j) temp_in[j] = input[j * stride + i] << 3;
#else
